...
function spawn(camera: PerspectiveCamera, scene: Scene) {
  ...
  switch (spawnMode) {
    ...
    case 2:
      ...
      break;
    case 3:
      if (spawnCycle % 3 != 2) spawnBox(camera, scene, (spawnCycle % 2) - 1);
      break;
  }
...
export function reset(
  ...
  scene: Scene,
  planeMesh: Mesh,
  gameStateStore: Store<GameState, Action>
) {
...
export default function tick(
  ...
  pickupMeshes = pickupMeshes.filter(mesh => {
    if (
      Math.abs(planeMesh.position.x - mesh.position.x) < 0.15 &&
